package net.yh;

import net.xn;
import net.yh.c;
import net.yh.x;

public class s extends x {
   float j;

   protected s(net.yv.r var1, double var2, double var4, double var6, double var8, double var10, double var12) {
      super(var1, var2, var4, var6, 0.0D, 0.0D, 0.0D);
      float var10000 = 2.5F;
      this.B *= 0.10000000149011612D;
      this.R *= 0.10000000149011612D;
      this.L *= 0.10000000149011612D;
      this.B += var8;
      this.R += var10;
      this.L += var12;
      float var15 = 1.0F - (float)(Math.random() * 0.30000001192092896D);
      this.N = var15;
      this.F = var15;
      this.V = var15;
      this.g *= 0.75F;
      this.g *= 2.5F;
      this.j = this.g;
      this.a = (int)(8.0D / (Math.random() * 0.8D + 0.3D));
      this.a = (int)((float)this.a * 2.5F);
   }

   public void A(net.y.p var1, net.ne.l var2, float var3, float var4, float var5, float var6, float var7, float var8) {
      float var9 = ((float)this.x + var3) / (float)this.a * 32.0F;
      var9 = net.u.t.T(var9, 0.0F, 1.0F);
      this.g = this.j * var9;
      super.A(var1, var2, var3, var4, var5, var6, var7, var8);
   }

   public void f() {
      this.i = this.K;
      x.f();
      this.m = this.C;
      this.O = this.A;
      if(this.x++ >= this.a) {
         this.Z();
      }

      this.Y(7 - this.x * 8 / this.a);
      this.f(this.B, this.R, this.L);
      this.B *= 0.9599999785423279D;
      this.R *= 0.9599999785423279D;
      this.L *= 0.9599999785423279D;
      net.r.r var2 = this.e.d(this.K, this.C, this.A, 2.0D, false);
      if(var2 != null) {
         net.u.e var3 = var2.i();
         if(Double.compare(this.C, var3.u) > 0) {
            this.C += (var3.u - this.C) * 0.2D;
            this.R += (var2.K - this.R) * 0.2D;
            this.P(this.K, this.C, this.A);
         }
      }

      if(this.T) {
         this.B *= 0.699999988079071D;
         this.L *= 0.699999988079071D;
      }

   }

   private static xn a(xn var0) {
      return var0;
   }

   public static class f implements c {
      public x v(int var1, net.yv.r var2, double var3, double var5, double var7, double var9, double var11, double var13, int... var15) {
         return new s(var2, var3, var5, var7, var9, var11, var13);
      }
   }
}
